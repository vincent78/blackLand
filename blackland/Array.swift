...
//            return unsafeBitCast(self, [U].self).indexOf(item)
            return self.indexOf{ (Element) in
                if let tmp:U! = Element as! U {
                    return tmp == item
                }  else {
                    return false
                }
...
//    }
     ...
     */
    public func partitionBy <T: Equatable> (cond: (Element) -> T) -> [Array] {
        var result = [Array]()
        var lastValue: T? = nil
     ...
     */
    public func unique <T: Equatable> () -> [T] {
        var result = [T]()
    ...
    /**
     ...
     :returns: The set of elements for which call(element) is unique
     */
    public func uniqueBy <T: Equatable> (call: (Element) -> (T)) -> [Element] {
        var result: [Element] = []
        var uniqueItems: [T] = []
...
//    }
     ...
     */
    public subscript (rangeAsArray: Range<Int>) -> Array {
